...
def get_elevation(lat, lon):
    ...
    data = response.json()
    return data['results'][0]['elevation']  # Devuelve solo el valor numérico de la altitud
...
def main():
    ...
    if weather_option == "Por coordenadas":
        ...
        if st.button("Obtener clima", key="get_weather_button"):
            ...
            st.markdown(f"<div class='info-box'>Altitud: {elevation}</div>", unsafe_allow_html=True)  # Solo el valor numérico
